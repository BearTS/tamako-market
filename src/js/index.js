$(function(){
  ...
  $.getJSON('https://raw.githubusercontent.com/BearTS/Mai/master/assets/json/market.json', function(data, status){
        ...
        <article class="item ${item.type}" sort-id="${parseInt(item.id)}" sort-price="${item.price}" sort-name="${item.name}">
          ...
          <div class="preview">
            ...
            <div class="overlay">
              ...
              <p> To buy: t!buy ${item.id} </p>
              <p> To equip: t!equipitem ${item.id} </p>
              </div>
            </div>
          </div>
          <div class="item-tags">
